...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, queries, keys, causality=False):
        # query, key e value for each sequence in the batch (for each item for a user)
        Q = self.query_proj(queries) # shape of queries [batch_dim, sequence_length, embedding_dim]
        K = self.key_proj(keys)
        V = self.val_proj(keys)

        # Split and concat (to achieve parallel computation for each head)
        ...
        # Multiplication (raw affinity computed using dot product)
        outputs = torch.matmul(Q_, K_.transpose(1, 2))

        # Scale
        outputs = outputs / (K_.size(-1) ** 0.5)

        # Key Masking (track 0s sequences and assign them -inf to have 0 after softmax)
        ...
        # Causality (avoid see the future, useful to predict only using the past)
        if causality:
        ...
        # Weighted sum (multiply each value for his attention, to have higher values for more relevant items)
        outputs = torch.matmul(outputs, V_)

        # Restore shape (same shape of the beginning)
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, hidden_dim, dropout_rate=0.5, causality=True):
        ...
        # instead of compute mean and variance between different examples in a batch (batch normalization), they are
        # computed on the features of a single element
    ...
    def forward(self, seq, mask=None): # seq: tensor containing the users processed in parallel (batch) and for each one
                                       # a sequence of items already consumed and the dimension of the embeddings of the item
        # normalized input as "queries" and unnormalized input as "keys"
        ...
        # Add & Norm
        x = x + queries     # residual (skip) connection: to mitigate the degradation of the gradient in deep network
