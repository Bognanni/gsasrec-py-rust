...
This is a modified version of the official gSASRec model. The architecture behind is the same of the original one, but some features are added like:
hyperparameters fine tuning, creation of an endpoint with FastApi and the most important thing is that the same model is developped also in Rust in
order to compare the performances in time and results. From this point is all defined by the original version, with credits and explanations about the
correct use.
